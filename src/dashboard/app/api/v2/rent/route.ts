...
export async function GET(req: NextRequest) {
  ...
  const corrected = rows.map(r => {
    ...
    const isPartial = amountPaid != null && amountPaid < rentAtTime;
    return { ...r, amount_due: rentAtTime, is_partial: isPartial };
  });
...
export async function POST(req: NextRequest) {
  ...
  const { unit_id, lease_id, due_date, amount_due, paid_date, amount_paid, notes, late_fee_charged, late_fee_paid, is_late } = body;
...
export async function PUT(req: NextRequest) {
  ...
  if (headerIdx === -1) return NextResponse.json({ error: 'Could not find CSV header row' }, { status: 400 });
  ...
  for (const line of lines.slice(headerIdx + 1)) {
    ...
    creditRows.push({ date: cols[dateIdx] ?? '', description: cols[descIdx] ?? '', amount });
  }

  return NextResponse.json({ batch_id: batchId, rows: creditRows, count: creditRows.length });
}
